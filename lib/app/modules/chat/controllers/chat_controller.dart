...
class ChatController extends GetxController {
  ...
  final Map<String, List<Map<String, dynamic>>> _bufferedIncomingReactions =
      {}; // realId -> list of reaction payloads
      ...
      if ((msg.senderId == receiverId && msg.receiverId == currentUserId) ||
        ...
        if (tempIndex != -1) {
          ...
          if (tempId != null && _pendingOutgoingReactions.containsKey(tempId)) {
            ...
            print(
              "Processing queued outgoing reaction for $tempId -> ${msg.id}",
            );
          ...
          if (echoTempId != null &&
              _pendingOutgoingReactions.containsKey(echoTempId)) {
            print(
              "Safety Net: Processing queued reaction for missing temp msg $echoTempId",
            );
  ...
  void _handleIncomingReaction(Map<String, dynamic> payload) {
    ...
    if (msgIndex != -1) {
      ...
      print(
        "Buffering incoming reaction for unknown message ID: $messageId",
      ); // DEBUG LOG
  ...
  void sendReaction(String messageId, String emoji) async {
    ...
    if (messageId.startsWith('temp_')) {
      print(
        "Queueing outgoing reaction for Temp ID: $messageId. Queue size: ${_pendingOutgoingReactions.length + 1}",
      );
  ...
  Future<void> sendImage(String path) async {
    ...
    if (imageUrl != null) {
      final payload = {"receiver_id": receiverId, "image_url": imageUrl};
      _socketService.sendMessage(payload);
    }
  }
